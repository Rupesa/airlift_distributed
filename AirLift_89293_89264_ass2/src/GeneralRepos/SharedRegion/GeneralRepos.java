...
/**
 * General Repository. Implements the LoggerInterface interface, with a logger
 * that writes the logs to a file specified on the SimulationParameters class.
 * Also has the intervining entities and the shared regions needed to print the
 * logs.
 */
public class GeneralRepos {
     ...
     */
    public synchronized void updateInfoBoardPlane(int nFlight, int nPassengers) {
     ...
     */
    public synchronized void updateHostessState(HostessState state, int id) {
        hostessState = state;
        currentPassenger = id;
        reportStatus();
    }
